; (function (factory) {
  ...
  // v1.0.0 - Limit title (...)
  EvoCalendar.prototype.limitTitle = function (title = "Evento sin nombre", limit) {
    // var newTitle = [];
  ...
  ****************/
  ...
  EvoCalendar.prototype.editCalendarEvent = function (eventData) {
    var _ = this;

    var i, eventIndex = -1;
    for (i = 0; i < _.options.calendarEvents.length; i++) {
      if (_.options.calendarEvents[i].id === eventData.id) {
        eventIndex = i;
        break;
      }
    }
    ...
    if (eventIndex !== -1) {
      _.options.calendarEvents[eventIndex] = eventData;

      _.addEventIndicator(eventData);
      _.buildEventList();
    }
  };
