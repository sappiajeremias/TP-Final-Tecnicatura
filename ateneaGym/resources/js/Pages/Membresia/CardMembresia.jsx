...
function CardMembresia({ membresia, auth }) {
  ...
  const { data, setData, put, post, processing, errors, reset, get } = useForm({
    membresia_id: id,
  });

  const [numberToWords, setNumberToWords] = useState(""); // Estado para almacenar el resultado SOAP
  ...
  const makeSoapRequest = async (num) => {
    try {
      ...
      const numberToWordsResult = xmlDoc.querySelector("NumberToWordsResult");
      ...
      if (numberToWordsResult) {
        ...
        console.log("Elemento <m:NumberToWordsResult> no encontrado en la respuesta XML.");
      }
    } catch (error) {
      console.error(error);
    }
